...
from dataset import CHECKPOINT_PATH, numpy_collate, test_loader, train_set, val_loader
from initialize import kaiming_init, init_simple_model
...
def load_model(model_path, model_name, state=None):
    ...
    # params = checkpoints.restore_checkpoint(model_file, target=net)
...
def save_model(model, params, model_path, model_name):
    ...
    # chkp = checkpoints.save_checkpoint(ckpt_dir=model_file, target=params, step=0)
    # print(chkp)
...
if __name__ == "__main__":
    ...
    model, params = init_simple_model(kaiming_init, exmp_imgs, act_fn=nn.relu)
    # _ = train_model(
    #     model,
    #     params,
    #     sgd(lr=1e-1),
    ...
    #     model,
    #     params,
    #     sgd_momentum(lr=1e-1, momentum=0.9),
    #     "FashionMNIST_SGDMom",
    #     max_epochs=40,
    #     batch_size=256,
        # overwrite=True,
    ...
    _ = train_model(model, params, adam(lr=1e-3),
                    "FashionMNIST_Adam",
                    max_epochs=40, batch_size=256, overwrite=True)
